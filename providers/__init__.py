...
Supports two automation modes (set via config.AUTOMATION_MODE):
...
# ─────────────────── Mode Dispatch ───────────────────
# Delegate ALL registry functions to the active backend.
# This keeps the consumer API unchanged (web_app.py, mcp_server.py).

_mode = getattr(config, "AUTOMATION_MODE", "phase1")

if _mode == "phase2":
    # Phase 2: OS-level automation (PyWinAuto + PyAutoGUI)
    from phase2.providers import (          # noqa: F401
        get_automator,
        get_all_statuses,
    ...
    print(f"[registry] Automation mode: PHASE 2 (PyWinAuto + PyAutoGUI)")
else:
    # Phase 1: Selenium/CDP (original)
    import json
    import threading
    ...
    print(f"[registry] Automation mode: PHASE 1 (Selenium/CDP)")
